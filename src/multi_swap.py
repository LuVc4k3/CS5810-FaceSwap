...
class face_vid():
    def __init__(self, vid_path, dummy=False):
        self.path = vid_path
        self.cap = cv2.VideoCapture(self.path)
        if not dummy:
            self.out = cv2.VideoWriter(
                self.path + "_multiswap.avi",
    ...
    def multi_landmark_detect(self, frame, face_i, use_optical_flow = False, show_landmark=False):
        self.convex_hull = []
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        self.mask = np.zeros_like(gray)
        rects = detector(gray)
        ...
        if len(rects) == 0:
            ...
            rects = sorted_faces(rects)

            shape = predictor(gray, rects[face_i])
...
if __name__ == '__main__':
    ...
    vid_A = face_vid('../resources/interview_two_faces.mp4')
    vid_B = face_vid('../resources/interview_two_faces.mp4', True)
    ...
    while (vid_A.cap.isOpened()):
        ...
        vid_A.multi_landmark_detect(frame_A, 0, True)
        vid_B.multi_landmark_detect(frame_B, 1, True)

        ''' Triangulation swap '''
        processed_frame = triangular_swap(
            vid_B,
            frame_B,
            vid_A,
            frame_A,
        ...
        processed_frame_f = triangular_swap(
            vid_A,
            frame_A,
            vid_B,
            processed_frame,
        ...
        vid_A.out.write(processed_frame_f)
    
        cv2.imshow("vidA", processed_frame_f)
